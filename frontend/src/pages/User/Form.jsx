...
export function Form() {
  ...
  }, [formTemplateId, hasUnsavedChanges, items.length, showRestoreDraftModal, departmentId, locationId, assetId, shiftId, location, asset, shift, answers, getDraftKey, saveDraft])
    ...
    ;(async () => {
      ...
      try {
        ...
        try {
          ...
          setShiftsList(Array.isArray(shifts) ? shifts : [])
  ...
  const handleRestoreDraft = () => {
    ...
    setLocation(draftToRestore.location || '')
    setAsset(draftToRestore.asset || '')
    setShift(draftToRestore.shift || '')
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      clearDraft()
      setSubmittedContext({ location: location.trim(), asset: asset.trim(), shift: shift.trim() })
              ...
              <div className="space-y-1.5">
                ...
                <select
                  ...
                  onChange={(e) => {
                    const id = e.target.value
                    setShiftId(id)
                    const s = shiftsList.find((x) => (x._id?.toString() || x._id) === id)
                    setShift(s ? (s.name || (s.startTime && s.endTime ? `${s.startTime}-${s.endTime}` : '')) : '')
                  }}
                  className={`w-full border rounded-md px-3 py-2 text-sm focus:ring-2 focus:ring-maroon-500 focus:border-maroon-500 transition-all ${duplicateExists ? 'border-red-500 bg-red-50' : 'border-slate-300 hover:border-slate-400'}`}
                  disabled={duplicateExists}
                  required
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6">
            <div className="text-center">
              ...
              <div className="text-sm text-slate-600 mb-4 space-y-1">
                <p><span className="font-semibold">Checklist submitted.</span> {submittedContext && [submittedContext.location, submittedContext.asset, submittedContext.shift].filter(Boolean).length > 0 && ` (${[submittedContext.location, submittedContext.asset, submittedContext.shift].filter(Boolean).join(', ')})`}</p>
              </div>
              <button
                onClick={() => {
